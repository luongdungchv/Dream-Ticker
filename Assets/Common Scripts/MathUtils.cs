...
namespace DL.Utils
{
    public static class MathUtils
    {
        ...
        public static float DistanceToSquareEnter(Vector2 from, Vector2 sqrCenter, float sqrExtend){
            var dir = sqrCenter - from;
            var length = dir.magnitude;

            var cos = Mathf.Abs(dir.x / length);
            var sin = Mathf.Abs(dir.y / length);

            if(sin > cos){
                return length - sqrExtend / sin;
            }
            else return length - sqrExtend / cos;
        }

        public static float DistanceToSquareExit(Vector2 from, Vector2 sqrCenter, float sqrExtend){
            var dir = sqrCenter - from;
            var length = dir.magnitude;
        ...
        public static bool IsCircleOverlapSquare(Vector2 circleCenter, float circleRadius, Vector2 sqrCenter, float sqrExtend){
            var shortDist = DistanceToSquareEnter(circleCenter, sqrCenter, sqrExtend);
            var longDist = DistanceToSquareEnter(circleCenter, sqrCenter, sqrExtend);

            sqrCenter = new Vector2(Mathf.Abs(sqrCenter.x), Mathf.Abs(sqrCenter.y));
            var shortCorner = Vector2.Distance(circleCenter, sqrCenter - Vector2.one * sqrExtend);
            var longCorner = Vector2.Distance(circleCenter, sqrCenter + Vector2.one * sqrExtend);

            shortDist = Mathf.Max(shortDist, shortCorner);
            longDist = Mathf.Max(longCorner, longDist);

            return shortDist < circleRadius && longDist > circleRadius;
        }
    }
}